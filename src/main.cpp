...
String topicCmndPower           = topicsRoot + "cmnd/POWER";
String topicCmndColor           = topicsRoot + "cmnd/COLOR";
String topicCmndEffect          = topicsRoot + "cmnd/EFFECT";
String topicCmndBrightness      = topicsRoot + "cmnd/BRIGHTNESS";
String topicCmndAnimationSpeed  = topicsRoot + "cmnd/ANIMATIONSPEED";

String topicStateColor          = topicsRoot + "COLOR";
String topicStatePower          = topicsRoot + "POWER";
String topicStateEffect         = topicsRoot + "EFFECT";
String topicStateBrightness     = topicsRoot + "BRIGHTNESS";
String topicStateAnimationSpeed = topicsRoot + "ANIMATIONSPEED";
...
String setColor = "150,0,150";
String setPower = "OFF";
String setEffect = "Solid";
int brightness = 200;
String setBrightness = String(brightness);
int animationspeed = 240;
String setAnimationSpeed = String(animationspeed);
String setColorTemp;
int Rcolor = 0;
int Gcolor = 0;
int Bcolor = 0;
...
IPAddress ip(10, 20, 30, 88);

EthernetClient ethernetClient;
PubSubClient client(ethernetClient); //this needs to be unique for each controller
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == topicCmndPower) {
    setPower = String(message_buff);
    Serial.println("Set Power: " + setPower);
    client.publish(topicStatePower.c_str(), setPower.c_str(), true);
  } else if (String(topic) == topicCmndEffect) {
    ...
    if (setEffect == "Shutdown") {
      fill_solid(leds, NUM_LEDS, CRGB(255, 0, 0)); // Red
      shutdownAt = millis() + (60 * 1000); // 1 minute
      // shutdownAt = millis() + (15 * 1000);
    }
    client.publish(topicStatePower.c_str(), setPower.c_str(), true);
    client.publish(topicStateEffect.c_str(), message_buff, true);
  } else if (String(topic) == topicCmndBrightness) {
    setBrightness = String(message_buff);
    Serial.println("Set Brightness: " + setBrightness);
    brightness = setBrightness.toInt();
    setPower = "ON";
    client.publish(topicStatePower.c_str(), setPower.c_str(), true);
    client.publish(topicStateBrightness.c_str(), message_buff, true);
  } else if (String(topic) == topicCmndColor) {
    setColor = String(message_buff);
    Serial.println("Set Color: " + setColor);
    setPower = "ON";
    client.publish(topicStatePower.c_str(), setPower.c_str(), true);
    client.publish(topicStateColor.c_str(), message_buff, true);
  } else if (String(topic) == topicCmndAnimationSpeed) {
    setAnimationSpeed = String(message_buff);
    animationspeed = setAnimationSpeed.toInt();
    Serial.println("Set AnimationSpeed: " + setAnimationSpeed);
    client.publish(topicStateAnimationSpeed.c_str(), message_buff, true);
  } else {
...
/////////////////////////////////////////  
/////// Metalab custom effects /////////
///////////////////////////////////////

  if (setEffect == "Shutdown") {                                  // Red Stripe fading out automatically after given time
    if(shutdownAt < millis()) {
      static unsigned int step = 0;
      step++;
      if(step % 20 == 0) { // fade out slowly
        for ( int i = 0; i < NUM_LEDS; i++) {
          leds[i].fadeToBlackBy( 1 );   //FADE OFF LEDS
        }
      }
    }
  }
...
/////////////////////////////////////////////
  ...
  if (setEffect == "Rainbow") {
    ...
    // static uint8_t starthue = 0; //unused!!
    thishue++;
  ...
  if (setEffect == "Lightning") {
    ...
    // Serial.println(twinklecounter); // Spammy ...
    if (twinklecounter < 2) {
      FastLED.clear();
      FastLED.show();
    }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(SENSORNAME, mqtt_user, mqtt_password, topicLWT.c_str(), 0, true, "offline")) {
      ...
      client.subscribe(topicCmndColor.c_str());
      client.subscribe(topicCmndBrightness.c_str());
      client.subscribe(topicCmndPower.c_str());
      client.subscribe(topicCmndEffect.c_str());
      client.subscribe(topicCmndAnimationSpeed.c_str());
      
      // Publish the states after connecting
      client.publish(topicStatePower.c_str(), setPower.c_str(), true);
      client.publish(topicStateColor.c_str(), setColor.c_str(), true);
      client.publish(topicStateEffect.c_str(), setEffect.c_str(), true);
      client.publish(topicStateAnimationSpeed.c_str(), String(animationspeed).c_str(), true);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // For testing the LEDs only (no network or mqtt required nor supported!)
  while(false) {
    Serial.println("Testing LED Setup only (no network support!): Red");
    fill_solid(leds, NUM_LEDS, CRGB(255, 0, 0));
    FastLED.show();
    delay(1000);

    Serial.println("Testing LED Setup only (no network support!): Green");
    fill_solid(leds, NUM_LEDS, CRGB(0, 255, 0));
    FastLED.show();
    delay(1000);

    Serial.println("Testing LED Setup only (no network support!): Blue");
